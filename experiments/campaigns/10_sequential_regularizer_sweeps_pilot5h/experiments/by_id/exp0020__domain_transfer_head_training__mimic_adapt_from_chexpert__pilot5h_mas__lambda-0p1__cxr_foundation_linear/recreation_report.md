...
- Experiment directory: `/tmp/cxr_sequential_regularizer_sweeps/experiments/by_id/exp0020__domain_transfer_head_training__mimic_adapt_from_chexpert__pilot5h_mas__lambda-0p1__cxr_foundation_linear`
...
- Token pooling: `avg`
- Head type: `linear`
- Init checkpoint: `/tmp/cxr_sequential_regularizer_sweeps/experiments/by_id/exp0019__domain_transfer_head_training__chexpert_adapt_from_nih__pilot5h_mas__lambda-0p1__cxr_foundation_linear/best.ckpt`
...
- MAS enabled: `True`
- MAS state path: `/tmp/cxr_sequential_regularizer_sweeps/mas_states/mas_lambda-0p1/stage_b_chexpert_mas_state.pt`
- MAS lambda: `0.1`
- MLP hidden dims: `[]`
- MLP dropout: `0.2`
...
python \
  ...
  --experiment-name \
  exp0020__domain_transfer_head_training__mimic_adapt_from_chexpert__pilot5h_mas__lambda-0p1__cxr_foundation_linear \
  --split-profile \
  mimic_adapt_from_chexpert \
  --init-checkpoint \
  /tmp/cxr_sequential_regularizer_sweeps/experiments/by_id/exp0019__domain_transfer_head_training__chexpert_adapt_from_nih__pilot5h_mas__lambda-0p1__cxr_foundation_linear/best.ckpt \
  --enable-mas \
  --mas-state-path \
  /tmp/cxr_sequential_regularizer_sweeps/mas_states/mas_lambda-0p1/stage_b_chexpert_mas_state.pt \
  --mas-lambda \
  0.1 \
  --embedding-root \
...
- `d0_test` macro AUROC `0.823630`, macro AP `0.223626`
- `d0_val` macro AUROC `0.826547`, macro AP `0.240613`
- `d1_test` macro AUROC `0.810163`, macro AP `0.516454`
- `d1_val` macro AUROC `0.721819`, macro AP `0.334784`
- `target_test` macro AUROC `0.777115`, macro AP `0.411996`
- `target_val` macro AUROC `0.802905`, macro AP `0.388655`

## Notes

- Training uses only `target_train` embeddings.
- Auxiliary training aliases: `[]`.
- Initialization checkpoint: `/tmp/cxr_sequential_regularizer_sweeps/experiments/by_id/exp0019__domain_transfer_head_training__chexpert_adapt_from_nih__pilot5h_mas__lambda-0p1__cxr_foundation_linear/best.ckpt`.
- LwF teacher checkpoint: `None`.
- MAS state path: `/tmp/cxr_sequential_regularizer_sweeps/mas_states/mas_lambda-0p1/stage_b_chexpert_mas_state.pt`.
