#!/usr/bin/env python3
"""Simple LoRA adaptation baseline from NIH to MIMIC."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LoRA DenseNet-121 adaptation from NIH to MIMIC.")
...
def resolve_cli_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.run_name is None:
        args.run_name = f"lora_seed{args.seed}"
...
class LoRALinear(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        lora_output = self.lora_up(self.lora_down(self.dropout(inputs))) * self.scaling
...
class LoRAConv2d(nn.Module):
    def __init__(self, base_layer: nn.Conv2d, rank: int, alpha: float, dropout: float) -> None:
        ...
        if base_layer.groups != 1:
            raise ValueError("This simple LoRAConv2d only supports groups=1 convolutions.")
...
def find_lora_target_module_names(model: nn.Module) -> list[str]:
    target_names: list[str] = []
    for name, module in model.named_modules():
        if name == "classifier" and isinstance(module, nn.Linear):
            target_names.append(name)
        elif name.startswith("features.denseblock4.") and name.endswith(("conv1", "conv2")) and isinstance(module, nn.Conv2d):
            target_names.append(name)
    return target_names
...
def inject_lora_modules(
    ...
    target_module_names = find_lora_target_module_names(model)
    if not target_module_names:
        raise StageFailure("No LoRA target modules were found in DenseNet-121.")
...
def summarize_parameter_trainability(
    ...
    missing_trainable_names: list[str] = []

    expected_trainable_names = set()
    for module_name in target_module_names:
        expected_trainable_names.add(f"{module_name}.lora_down.weight")
        expected_trainable_names.add(f"{module_name}.lora_up.weight")
    ...
    for name in sorted(expected_trainable_names):
        if name not in trainable_parameter_names:
            missing_trainable_names.append(name)

    return {
        "adaptation_method": "lora_lastblock",
        "lora_rank": int(lora_rank),
        "lora_alpha": float(lora_alpha),
        "lora_dropout": float(lora_dropout),
        "target_module_names": target_module_names,
        "target_module_count": int(len(target_module_names)),
...
def verify_lora_trainability(summary: dict[str, Any]) -> None:
    if summary["target_module_count"] == 0:
        raise StageFailure("LoRA did not target any modules.")
    if summary["trainable_parameter_tensors"] == 0:
        raise StageFailure("No LoRA parameters are trainable.")
    if summary["unexpected_trainable_names"]:
        raise StageFailure(
            "Some non-LoRA parameters are unexpectedly trainable: "
            f"{summary['unexpected_trainable_names'][:20]}"
        )
    if summary["missing_trainable_names"]:
        raise StageFailure(
            "Some LoRA parameters are unexpectedly frozen: "
            f"{summary['missing_trainable_names'][:20]}"
        )
    if not any(name.endswith("classifier.lora_up.weight") for name in summary["trainable_parameter_names"]):
        raise StageFailure("Classifier LoRA parameters are not trainable.")
...
def save_loss_curve(train_losses: list[float], val_losses: list[float], output_path: Path) -> str:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("LoRA adaptation loss curve")
    plt.legend()
    plt.tight_layout()
...
def initial_report(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "goal": "Adapt the NIH-trained DenseNet-121 model to MIMIC using LoRA on denseblock4 and classifier.",
        "run_name": args.run_name,
        "adaptation_method": "lora_lastblock",
...
def build_markdown_report(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- target module count: {report['parameter_summary'].get('target_module_count')}",
            f"- target module names sample: {report['parameter_summary'].get('target_module_names', [])[:10]}",
...
def build_lora_model_from_saved_state(
    ...
    target_module_names = inject_lora_modules(model, rank=rank, alpha=alpha, dropout=dropout)
...
def run_adaptation(args: argparse.Namespace, report: dict[str, Any]) -> dict[str, Any]:
    ...
    if args.lora_rank <= 0:
        raise StageFailure(f"LoRA rank must be positive, got {args.lora_rank}")
    ...
    target_module_names = inject_lora_modules(
        model,
        rank=args.lora_rank,
    ...
    print(f"LoRA target modules: {parameter_summary['target_module_count']}")
    ...
    verify_lora_trainability(parameter_summary)
    model = model.to(device)
    ...
    if not isinstance(saved_checkpoint, dict) or "model_state_dict" not in saved_checkpoint:
        raise StageFailure(f"Best LoRA checkpoint is missing model_state_dict: {best_checkpoint_path}")
    best_model = build_lora_model_from_saved_state(
        state_dict=saved_checkpoint["model_state_dict"],
        rank=args.lora_rank,
        alpha=args.lora_alpha,
        dropout=args.lora_dropout,
        device=device,
