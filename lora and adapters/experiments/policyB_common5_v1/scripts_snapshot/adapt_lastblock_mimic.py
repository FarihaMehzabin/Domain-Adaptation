#!/usr/bin/env python3
"""Last-block plus head adaptation baseline from NIH to MIMIC."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Last-block DenseNet-121 adaptation from NIH to MIMIC.")
...
def resolve_cli_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.run_name is None:
        args.run_name = f"lastblock_seed{args.seed}"
...
def is_lastblock_trainable_name(name: str) -> bool:
...
def summarize_parameter_trainability(model: nn.Module) -> dict[str, Any]:
    ...
    unexpected_frozen_names: list[str] = []

    for name, parameter in model.named_parameters():
        parameter_names.append(name)
        total_parameters += parameter.numel()

        should_be_trainable = is_lastblock_trainable_name(name)
        if parameter.requires_grad:
            ...
            if should_be_trainable:
                unexpected_frozen_names.append(name)
    ...
    return {
        ...
        "unexpected_frozen_names": unexpected_frozen_names,
    }


def enable_lastblock_finetune(model: nn.Module) -> dict[str, Any]:
    for name, parameter in model.named_parameters():
        parameter.requires_grad = is_lastblock_trainable_name(name)
    return summarize_parameter_trainability(model)


def verify_lastblock_trainability(summary: dict[str, Any]) -> None:
    trainable_names = summary["trainable_parameter_names"]
    unexpected_trainable_names = summary["unexpected_trainable_names"]
    unexpected_frozen_names = summary["unexpected_frozen_names"]

    if summary["trainable_parameter_tensors"] == 0:
        raise StageFailure(
            "No trainable parameters remain, but last-block adaptation requires denseblock4, norm5, and classifier."
        )
    if unexpected_trainable_names:
        raise StageFailure(
            "Some earlier backbone parameters are unexpectedly trainable during last-block adaptation: "
            f"{unexpected_trainable_names[:20]}"
        )
    if unexpected_frozen_names:
        raise StageFailure(
            "Some required last-block or classifier parameters are unexpectedly frozen: "
            f"{unexpected_frozen_names[:20]}"
        )

    required_names = {"classifier.weight", "classifier.bias"}
    missing_names = sorted(required_names - set(trainable_names))
    if missing_names:
        raise StageFailure(
            "Classifier parameters are not trainable during last-block adaptation: "
            f"{missing_names}"
        )
    ...
    for prefix in TRAINABLE_PREFIXES:
        if not any(name.startswith(prefix) for name in trainable_names):
            raise StageFailure(
                "Expected trainable parameter group is missing from the last-block mask: "
                f"{prefix}"
            )


def set_lastblock_train_mode(model: nn.Module) -> None:
    model.eval()
    model.features.denseblock4.train()
    model.features.norm5.train()
...
def train_one_epoch(
    ...
    set_lastblock_train_mode(model)
    total_loss = 0.0
    total_items = 0
...
def save_loss_curve(train_losses: list[float], val_losses: list[float], output_path: Path) -> str:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Last-block adaptation loss curve")
    plt.legend()
    plt.tight_layout()
...
def checkpoint_payload(
    ...
    return {
        ...
        "adaptation_method": "lastblock_head",
...
def initial_report(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "goal": "Adapt the NIH-trained DenseNet-121 model to MIMIC using denseblock4, norm5, and classifier fine-tuning.",
        "run_name": args.run_name,
        "adaptation_method": "lastblock_head",
...
def build_markdown_report(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- trainable prefixes: {report['parameter_summary'].get('trainable_prefixes')}",
...
def run_adaptation(args: argparse.Namespace, report: dict[str, Any]) -> dict[str, Any]:
    ...
    parameter_summary = enable_lastblock_finetune(model)
    ...
    verify_lastblock_trainability(parameter_summary)
    ...
    reloaded_summary = enable_lastblock_finetune(best_model)
    verify_lastblock_trainability(reloaded_summary)
