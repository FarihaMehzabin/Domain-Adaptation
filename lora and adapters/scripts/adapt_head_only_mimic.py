...
MASK_COLUMNS = get_mask_columns(LABELS)
...
class MIMICDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        mask_tensor = torch.tensor(row[MASK_COLUMNS].values.astype(np.float32), dtype=torch.float32)
...
def validate_manifest(
    ...
    try:
        targets_np, masks_np = extract_targets_and_masks(dataframe, LABELS, label_policy)
    except ValueError as exc:
        raise StageFailure(f"{manifest_path}: {exc}") from exc

    for label_index, label in enumerate(LABELS):
        dataframe[label] = targets_np[:, label_index].astype(np.int64, copy=False)
        dataframe[f"{label}_mask"] = masks_np[:, label_index].astype(np.int64, copy=False)
    ...
    for label_index, label in enumerate(LABELS):
        ...
        if positives + negatives + masked != total_rows:
            raise StageFailure(
                f"{manifest_path}: {label} count mismatch "
                f"(positives={positives}, negatives={negatives}, masked={masked}, rows={total_rows})"
            )
        label_counts[label] = {
            "positives": positives,
            "negatives": negatives,
...
def print_split_summary(summary: dict[str, Any]) -> None:
    ...
    print(f"[{summary['split_name']}] label_policy: {summary.get('label_policy', 'n/a')}")
...
def compute_binary_metrics(targets: np.ndarray, probabilities: np.ndarray, masks: np.ndarray) -> dict[str, Any]:
    return compute_masked_multilabel_metrics(targets, probabilities, masks, LABELS)
...
def evaluate_split(
    ...
    metrics = compute_binary_metrics(targets_np, probabilities_np, masks_np)
    metrics["loss"] = float(total_loss / total_valid) if total_valid > 0 else 0.0
...
def save_predictions_csv(
    ...
    for row_index, row in dataframe.reset_index(drop=True).iterrows():
        ...
        for label in LABELS:
            ...
            record[f"{label}_mask"] = int(row[f"{label}_mask"])
        for label_index, label in enumerate(LABELS):
...
def empty_split_summary(split_name: str) -> dict[str, Any]:
    return {
        ...
        "num_dicoms": None,
        "label_counts": {
            label: {"positives": 0, "negatives": 0, "masked": 0, "n_valid": 0}
            for label in LABELS
        },
    }
