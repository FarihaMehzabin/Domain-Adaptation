...
def enforce_policy_b_manifest_guard(
    ...
    *,
    ...
    if support_manifest is not None:
        basename = support_manifest.name
        if basename.startswith("mimic_common5") and not basename.startswith("mimic_common5_policyB_support_"):
            raise ValueError(
                "--label_policy uignore_blankzero requires a Policy B support manifest, "
                f"but resolved {support_manifest}."
            )
