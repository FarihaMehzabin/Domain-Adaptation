...
from scripts.adapt_full_finetune_mimic import (  # noqa: E402
    LABELS,
    StageFailure,
    build_model,
    check_leakage,
    ...
    train_one_epoch,
    verify_full_finetune_trainability,
)
...
def run_single_training_step() -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    model = build_model()
    summary = enable_full_finetune(model)
    verify_full_finetune_trainability(summary)
...
def test_full_finetuning_leaves_all_parameters_trainable() -> None:
    model = build_model()

    summary = enable_full_finetune(model)
    verify_full_finetune_trainability(summary)

    assert summary["frozen_parameter_names"] == []
    assert summary["trainable_parameter_tensors"] == summary["total_parameter_tensors"]
    assert "classifier.weight" in summary["trainable_parameter_names"]
    assert "features.conv0.weight" in summary["trainable_parameter_names"]
...
def test_one_training_step_changes_backbone_weights() -> None:
    _, _, backbone_before, backbone_after = run_single_training_step()

    assert not torch.allclose(backbone_before, backbone_after)


def test_trainability_check_fails_if_parameter_is_frozen() -> None:
    model = build_model()
    ...
    with pytest.raises(StageFailure, match="unexpectedly non-trainable"):
        verify_full_finetune_trainability(summary)
