#!/usr/bin/env python3
"""Generate DINOv2 embeddings for the NIH CXR14 val/test splits.

This is a self-contained replacement for the existing thin DINOv2 wrapper
around the generic image-embedding script. The embedding setup matches the
existing DINOv2 run in this workspace:

- model: ``facebook/dinov2-large``
- pooling: CLS token
- normalization: L2 normalize each embedding
- outputs: ``embeddings.npy``, manifest CSV, image path list, failures JSONL,
...
try:
    ...
    raise SystemExit(
        "Missing dependency 'numpy'. Install the image embedding requirements first."
    ) from exc
...
try:
    ...
    raise SystemExit(
        "Missing dependency 'torch'. Install the image embedding requirements first."
    ) from exc
...
try:
    ...
    raise SystemExit(
        "Missing dependency 'pillow'. Install the image embedding requirements first."
    ) from exc
...
try:
    ...
    raise SystemExit(
        "Missing dependency 'transformers'. Install the image embedding requirements first."
    ) from exc


DEFAULT_DATA_ROOT = Path("/workspace/data")
DEFAULT_SPLIT_CSV_DIR = DEFAULT_DATA_ROOT / "nih_cxr14" / "splits"
DEFAULT_OUTPUT_ROOT = Path("/workspace/outputs/nih_cxr14_dinov2_large_embeddings")
...
def _normalize_extensions(raw_extensions: list[str]) -> set[str]:
    ...
    *,
    ...
    with split_csv_path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            ...
            resolved = _resolve_image_path(data_root, image_path)
...
def _pool_embeddings(model_outputs: Any, pooling: str) -> torch.Tensor:
...
def _save_manifest_csv(path: Path, image_paths: list[str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _save_failed_jsonl(path: Path, failures: list[dict[str, str]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    *,
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader, start=1):
            ...
            pooled = _pool_embeddings(outputs, pooling)
...
def _save_split_outputs(
    ...
    *,
    ...
    _save_manifest_csv(manifest_csv_path, embedded_paths)
    image_paths_txt_path.write_text("\n".join(embedded_paths) + "\n", encoding="utf-8")
    if failed_images:
        _save_failed_jsonl(failures_path, failed_images)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate DINOv2 image embeddings for NIH CXR14 val/test splits."
    )
    parser.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    parser.add_argument("--split-csv-dir", type=Path, default=DEFAULT_SPLIT_CSV_DIR)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--splits", nargs="+", choices=["val", "test"], default=DEFAULT_SPLITS)
...
def main() -> None:
    ...
    image_extensions = _normalize_extensions(args.extensions)
    splits = _dedupe_preserve_order(list(args.splits))
    ...
    for split in splits:
        ...
        print(
            f"[info] split={split} images={len(image_paths)} "
            f"split_csv={split_csv_path}"
        )

        embeddings, embedded_paths, failed_images = _embed_split(
            split,
            image_paths,
            processor,
            model,
